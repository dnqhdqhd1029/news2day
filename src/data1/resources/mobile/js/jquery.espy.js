...
(function($, w, undefined) {
	...
	$[pluginClass] = function(context, callback, options) {
		...
		function check(element) {
			...
			if (spy.contain) {
				...
				if (
					triggerStart > spy.start && triggerStart < spy.end || triggerEnd > spy.start && triggerEnd < spy.end || triggerStart <= spy.start && triggerEnd >= spy.start || triggerStart <= spy.end && triggerEnd >= spy.end) {
					newState = 'inside';
	...
	function throttle(delay, callback) {
		...
		function wrapper() { /*jshint validthis:true */
			var that = this;
	...
	$.fn[pluginName].defaults = {
		delay: 100,
		// Events throttling delay in milliseconds.
		context: window,
		// Scrolling context.
		horizontal: 0,
		...
		size: '100%',
		// Target area size (height in vert., width in hor.).
